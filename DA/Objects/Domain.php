 ...
 */
class DA_Objects_Domain extends DA_Objects_Object
{
     ...
     * @param DA_Context_UserContext $context The owning user context.
     * @param string|array $config The basic config string as returned by CMD_API_ADDITIONAL_DOMAINS.
     */
    public function __construct($name, DA_Context_UserContext $context, $config)
    {
        ...
        if($data['username'] === $context->getUsername())
            ...
            throw new DA_DirectAdminException('Could not determine relationship between context user and domain');
     ...
     */
    public static function create(DA_Objects_Users_User $user, $domainName, $bandwidthLimit = null, $diskLimit = null, $ssl = null, $php = null, $cgi = null)
    {
        $options = [
            'action' => 'create',
            'domain' => $domainName,
            ...
            'ssl' => DA_Utility_Conversion::onOff($ssl, $user->hasSSL()),
            'php' => DA_Utility_Conversion::onOff($php, $user->hasPHP()),
            'cgi' => DA_Utility_Conversion::onOff($cgi, $user->hasCGI()),
        ];
     ...
     * @return DA_Objects_Email_Forwarder The newly created forwarder.
     */
    public function createForwarder($prefix, $recipients)
    {
        return DA_Objects_Email_Forwarder::create($this, $prefix, $recipients);
    }
     ...
     * @return DA_Objects_Email_Mailbox The newly created mailbox.
     */
    public function createMailbox($prefix, $password, $quota = null, $sendLimit = null)
    {
        return DA_Objects_Email_Mailbox::create($this, $prefix, $password, $quota, $sendLimit);
    }
     ...
     * @return DA_Objects_Domains_Subdomain The newly created subdomain.
     */
    public function createSubdomain($prefix)
    {
        return DA_Objects_Domains_Subdomain::create($this, $prefix);
    }
    ...
    /**
     * @return DA_Objects_Email_Forwarder[] Associative array of forwarders.
     */
    public function getForwarders()
    {
        return $this->getCache(self::CACHE_FORWARDERS, function() {
            ...
            return DA_Objects_DomainObject::toDomainObjectArray($forwarders, DA_Objects_Email_Forwarder::class, $this);
        });
    }

    /**
     * @return DA_Objects_Email_Mailbox[] Associative array of mailboxes.
     */
    public function getMailboxes()
    {
        return $this->getCache(self::CACHE_MAILBOXES, function() {
            ...
            return DA_Objects_DomainObject::toDomainObjectArray($boxes, DA_Objects_Email_Mailbox::class, $this);
        });
    }
    ...
    /**
     * @return DA_Objects_Domains_Subdomain[] Associative array of subdomains.
     */
    public function getSubdomains()
    {
        return $this->getCache(self::CACHE_SUBDOMAINS, function() {
            ...
            return DA_Objects_DomainObject::toDomainObjectArray($subs, DA_Objects_Domains_Subdomain::class, $this);
        });
    }
